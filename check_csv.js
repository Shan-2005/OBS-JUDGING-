...
// Export json format for embedding into store.js
const jsonSeeding = teams.map(t => ({
  botId: t.botId,
  name: t.name,
  rosterSize: t.rosterSize,
  leader: t.leader || `Lead ${t.index}`
}));

fs.writeFileSync('d:\\obs_judging\\parsed_teams.json', JSON.stringify(jsonSeeding, null, 2));
